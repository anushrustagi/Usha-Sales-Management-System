...
const Header: React.FC<HeaderProps> = ({ activeView, setActiveView, businessName, data, onSearchClick }) => {
  ...
  const handleAiSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const context = {
        topCustomers: data.customers.sort((a,b) => b.outstandingBalance - a.outstandingBalance).slice(0,5),
        inventoryStats: {
          totalProducts: data.products.length,
          lowStock: data.products.filter(p => p.stock <= p.minStockAlert).length
        },
        recentSales: data.invoices.filter(i => i.type === 'SALE').slice(-5)
      };
      const prompt = `Context: ${JSON.stringify(context)} Question: ${aiQuery}`;
      const response = await ai.models.generateContent({ model: 'gemini-3-flash-preview', contents: prompt });
      setAiResponse(response.text || "No insights found.");
    } catch (err) { setAiResponse("Connection error."); } finally { setIsAiLoading(false); }
  };
  ...
  return (
    <header className="h-20 bg-white/80 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-10 shadow-sm relative sticky top-0">
      <div className="flex items-center gap-8">
        <div className="flex items-center gap-3 text-slate-400 pr-8 border-r border-slate-200">
          {data.companyProfile.logo ? (
            ...
            <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black">U</div>
          )}
      ...
      <div className="flex items-center gap-4">
        <div className="hidden xl:flex items-center gap-3 px-4 py-2 bg-emerald-50/50 border border-emerald-100 rounded-2xl">
        ...
        <button onClick={() => setShowAiSearch(true)} className="flex items-center gap-3 px-5 py-2.5 bg-slate-900 text-white rounded-2xl hover:bg-blue-600 transition-all shadow-xl shadow-slate-200 group">
        ...
        <div className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl border transition-all duration-300 ${isOnline ? 'bg-white border-emerald-100 text-emerald-600' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-xl shadow-2xl overflow-hidden animate-in zoom-in duration-200 border border-white/20">
            ...
            <div className="p-8 space-y-6">
              <form onSubmit={handleAiSearch} className="flex gap-3">
                <input autoFocus type="text" placeholder="e.g. 'Identify payment risks this month'" className="flex-1 bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:border-blue-400 focus:bg-white transition-all" value={aiQuery} onChange={e => setAiQuery(e.target.value)} />
                <button disabled={isAiLoading} className="bg-blue-600 text-white px-6 rounded-2xl shadow-xl hover:bg-blue-700 transition-all active:scale-95 disabled:opacity-50">{isAiLoading ? <Loader2 className="animate-spin" /> : <Send size={22}/>}</button>
              </form>
              <div className={`p-8 rounded-3xl border-2 border-slate-100 min-h-[200px] max-h-[400px] overflow-y-auto ${aiResponse ? 'bg-white shadow-inner' : 'bg-slate-50/50 flex items-center justify-center'}`}>
                {aiResponse ? <div className="prose prose-sm font-medium text-slate-700 whitespace-pre-line leading-relaxed text-xs uppercase tracking-tight">{aiResponse}</div> : <div className="text-center text-slate-300 italic flex flex-col items-center gap-4"><MessageSquare size={48} className="opacity-10" /><p className="text-[10px] uppercase font-black tracking-[0.3em]">Awaiting Strategic Query</p></div>}
              </div>
            </div>
